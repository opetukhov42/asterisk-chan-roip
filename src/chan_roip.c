/*
 * chan_roip -- Radio over IP (RoIP) channel driver for Asterisk
 *
 * Copyright (C) 2010, Bogdan Diaconescu
 *
 * Bogdan Diaconescu, YO3IIU <yo3iiu@yo3iiu.ro>
 * Based upon work of:
 ...
 * Upstream this is derived from: https://github.com/gaeldb/chan_alsaradio/
 *
 * Renamed to chan_roip and refocused as a general-purpose Radio-over-IP
 * channel component (ALSA audio + serial PTT/COR), ported to Asterisk 20.
 * The repeater/ham-grid heritage of the original (app_rpt companion use) is
 * intentionally not required: chan_roip is meant to work as a plain
 * point-to-point or bridged RoIP endpoint on its own.
 *
...
/*! \file
 *
 * \brief Radio over IP (RoIP) channel driver: ALSA audio with serial PTT/COR control
 *
...
/* ASTERISK_FILE_VERSION was removed in Asterisk 15. Guard it so this source
   builds unchanged on Asterisk 13 (macro present) through 20 (macro absent). */
#ifdef ASTERISK_FILE_VERSION
...
[general]
	...
	; [roip] generally (although its up to you) being the first device.
	;
	;
	...
	; jbenable = yes          ; Enables the use of a jitterbuffer on the receiving side of an
                                  ; roip channel. Defaults to "no". An enabled jitterbuffer will
                                  ; be used only if the sending side can create and the receiving
                                  ; side can not accept jitter. The roip channel can't accept jitter,
                                  ; thus an enabled jitterbuffer on the receive roip side will always
	...
	; jbimpl = fixed              ; Jitterbuffer implementation, used on the receiving side of an roip
...
context=from-roip		; dialplan context for COR-originated calls
extension=s			; extension to start at
cor_start_call=0		; when 1, originate a call on COR (carrier) rising edge
cor_hangup=0			; when 1, hang up that COR-originated call when carrier drops

ptt_on_call=0			; NEW: when 1, key PTT for the whole duration of a call
				; (e.g. Dial(roip/roip0)). PTT is released
				; automatically on hangup. Requires serial enabled and
				; honors force_hardware_eptt / invertptt.
 ...
 *		Overridden by the 'frags' parameter in roip.conf
 *
 ...
 *		Overridden by the 'queuesize' parameter in roip.conf
 *
...
static char *config = "roip.conf";					/* default config file */
//static char *config1 = "roip_tune_%s.conf";	/* tune config file */
 ...
 */
struct chan_roip_pvt {
	struct chan_roip_pvt		*next;
	char 					*name;
...
#define	BOOST_MAX				40	/* slightly less than 7 bits */
	...
	/* buffers used in roip_write, 2 per int */
	char 					roip_write_buf[FRAME_SIZE * 2];    

	int 					roip_write_dst;
	/* buffers used in roip_read - AST_FRIENDLY_OFFSET space for headers
	 * plus enough room for a full frame
	 */
	char 					roip_read_buf[FRAME_SIZE * 4 * 6];
	char 					roip_read_frame_buf[FRAME_SIZE * 2 + AST_FRIENDLY_OFFSET];
	...
	struct 					ast_frame read_f;/* returned by roip_read */
	...
	int 					cor_start_call;	/* originate a PBX call on COR rising edge */
	int 					cor_hangup;		/* hang up the COR call when COR drops */
	char 					corcall;		/* current owner channel was COR-originated */

	int 					ptt_on_call;	/* config: key PTT for the whole duration of an active call */
	char 					pttcall;		/* state: PTT currently forced on because a call is up */
...
/* the active device */
static char 				*roip_active;


/*	DECLARE FUNCTION PROTOTYPES	*/
//static void 				mixer_write(struct chan_roip_pvt *o);
static struct ast_channel 			*roip_request(const char *type, struct ast_format_cap *cap, const struct ast_assigned_ids *assignedids, const struct ast_channel *requestor, const char *data, int *cause);
static int 					roip_digit_begin(struct ast_channel *c, char digit);
static int 					roip_digit_end(struct ast_channel *c, char digit, unsigned int duration);
static int 					roip_text(struct ast_channel *c, const char *text);
static int 					roip_hangup(struct ast_channel *c);
static int 					roip_answer(struct ast_channel *c);
static struct ast_frame 			*roip_read(struct ast_channel *chan);
static int 					roip_call(struct ast_channel *c, const char *dest, int timeout);
static int 					roip_write(struct ast_channel *chan, struct ast_frame *f);
static int 					roip_indicate(struct ast_channel *chan, int cond, const void *data, size_t datalen);
static int 					roip_fixup(struct ast_channel *oldchan, struct ast_channel *newchan);
static int 					roip_setoption(struct ast_channel *chan, int option, void *data, int datalen);
static int 					setformat(struct chan_roip_pvt *o, int mode);

static snd_pcm_t 				*alsa_card_init(struct chan_roip_pvt *o, char *dev, snd_pcm_stream_t stream);
static void 					alsa_card_uninit(struct chan_roip_pvt *o);
static int					alsa_write(struct chan_roip_pvt *o, struct ast_frame *f);
static struct ast_frame 			*alsa_read(struct chan_roip_pvt *o);

static int 					serial_init(struct chan_roip_pvt *o);
static void 					serial_uninit(struct chan_roip_pvt *o);
static int 					serial_getcor(struct chan_roip_pvt *o);
static void 				roip_cor_event(struct chan_roip_pvt *o, int carrier);
static int 					serial_pttkey(struct chan_roip_pvt *o, enum ptt_status);
static int 					manage_ptt(struct chan_roip_pvt *o, enum ptt_status);
static int      				send_command(struct chan_roip_pvt *o, const char *cmd);
...
static struct ast_channel_tech
							roip_tech = {
	.type = "roip",
	.description = "Radio over IP (RoIP) channel driver",
	.requester = roip_request,
	.send_digit_begin = roip_digit_begin,
	.send_digit_end = roip_digit_end,
	.send_text = roip_text,
	.hangup = roip_hangup,
	.answer = roip_answer,
	.read = roip_read,
	.call = roip_call,
	.write = roip_write,
	.indicate = roip_indicate,
	.fixup = roip_fixup,
	.setoption = roip_setoption,
};
 ...
 */
static void						kickptt(struct chan_roip_pvt *o)
{
	char 						c;
	ssize_t						ret;
...
/*
 * Parse the chained list roip_default and returns a pointer
 * to the descriptor with the given dev name.
 */
static struct chan_roip_pvt *find_desc(const char *dev)
{
	struct chan_roip_pvt 	*o = NULL;

	if (!dev)
		ast_log(LOG_WARNING, "null dev\n");

	for (o = roip_default.next; o && o->name && dev && strcmp(o->name, dev) != 0; o = o->next);
	if (!o)
	{
...
static void 					*serthread(void *arg)
{
	...
	unsigned char 				txreq;
	int 						res;
	struct chan_roip_pvt 	*o = (struct chan_roip_pvt *) arg;
	struct timeval 				to;
	ast_fdset 					rfds;
	ssize_t 					bytes_read;
    ...
    while(!o->stopser)
    {
		...
		*/
		while (!o->stopser)
		{
			if (o->rxcdtype == SERIAL_DSR || o->rxcdtype == SERIAL_DSR_INVERT)
			{
				/* poll fast enough for hardware COR detection */
				to.tv_sec = 0;
				to.tv_usec = 50000;
			}
			else
		...
		return (0);*/
			...
			/* Hardware COR via serial DSR line (restored from KR4DIO original).
			   Only active when carrierfrom=serialdsr / serialdsrinvert. */
			if (o->rxcdtype == SERIAL_DSR || o->rxcdtype == SERIAL_DSR_INVERT)
			{
				keyed = sim_cor || serial_getcor(o);
				if (keyed != o->rxsersq)
				{
					ast_log(LOG_NOTICE, "chan_roip() serthread: update rxsersq = %d\n", keyed);
					o->rxsersq = keyed;
					/* rxsersq -> actual carrier state depends on polarity */
					roip_cor_event(o,
						(o->rxcdtype == SERIAL_DSR_INVERT) ? !keyed : keyed);
				}
			}
			...
			txreq = txreq || o->pttcall;	// NEW: ptt_on_call forces PTT for the whole call
			if (txreq && (!o->lasttx))
...
static int 				    manage_ptt(struct chan_roip_pvt *o, enum ptt_status ptt)
{
	if (ptt == PTT_ON)
	{
...
static int 						roip_text(struct ast_channel *c, const char *text)
{
	...
	o = ast_channel_tech_pvt(c);
	if (o->debuglevel)
		ast_verbose(" << Console Received roip text %s >> \n", text);
	return 0;
}

/* Play ringtone 'x' on device 'o' */
static void 					ring(struct chan_roip_pvt *o, int x)
{
	return;
}
...
static int 						roip_call(struct ast_channel *c, const char *dest, int timeout)
{
	...
	/* NEW: if ptt_on_call is set, force PTT keyed for the whole duration of the call.
	   pttcall feeds both the serial-thread PTT gate and the roip_write() audio gate.
	   kickptt() wakes the serial thread so PTT asserts immediately (otherwise up to the
	   3s idle select() timeout). Un-keying happens unconditionally in roip_hangup(). */
	if (o->ptt_on_call)
	{
		if (o->debuglevel)
			ast_verbose("== " ANSI_COLOR_YELLOW "ptt_on_call: keying PTT on %s for incoming call" ANSI_COLOR_RESET "\n", o->name);
		o->pttcall = 1;
		kickptt(o);
	}
	return 0;
...
static int 						roip_answer(struct ast_channel *c)
{
	ast_log(LOG_NOTICE, "roip_answer()\n");
	ast_setstate(c, AST_STATE_UP);
	return 0;
}
...
static int 						roip_hangup(struct ast_channel *c)
{
	struct chan_roip_pvt 	*o = ast_channel_tech_pvt(c);

	if (o->debuglevel)
		ast_log(LOG_NOTICE, "roip_hangup()\n");
	ast_channel_tech_pvt_set(c, NULL);
	o->owner = NULL;
	o->corcall = 0;
	/* NEW: always drop a ptt_on_call-forced PTT on hangup. This is unconditional by
	   design: leaving a transmitter keyed after the call ends would jam the channel
	   and can overheat the radio's PA, so there is deliberately no "keep PTT" option. */
	if (o->pttcall)
	{
		o->pttcall = 0;
		kickptt(o);
...
static int roip_write(struct ast_channel *c, struct ast_frame *f)
{
	struct chan_roip_pvt *o = ast_channel_tech_pvt(c);
	struct ast_frame *f1;
	int i, n;
	...
	o->tv2 = tv2;
#endif
	//ast_log(LOG_ERROR, "roip_write\n");

	/* NEW: pttcall opens the audio gate too. Without this, ptt_on_call would key the
	   transmitter but roip_write() would still drop every frame -> dead air. */
	if (!o->txkeyed && !o->pttcall) return 0;
...
static struct ast_frame *roip_read(struct ast_channel *c)
{
	int cd,sd;
	struct chan_roip_pvt *o = ast_channel_tech_pvt(c);
...
static int 						roip_fixup(struct ast_channel *oldchan, struct ast_channel *newchan)
{
	struct chan_roip_pvt 	*p;

	p = ast_channel_tech_pvt(newchan);
	ast_mutex_lock(&alsalock);
	ast_log(LOG_WARNING,"roip_fixup()\n");
	p->owner = newchan;
	ast_mutex_unlock(&alsalock);
	return 0;
}

static int 						roip_indicate(struct ast_channel *c, int cond, const void *data, size_t datalen)
{
	struct chan_roip_pvt	*o;
	int  						res;
 ...
 */
static struct ast_channel *roip_new(struct chan_roip_pvt *o, int state, const struct ast_assigned_ids *assignedids, const struct ast_channel *requestor)
{
	struct ast_channel *c = NULL;

	if (!(c = ast_channel_alloc(1, state, 0, 0, "", o->ext, o->ctx, assignedids, requestor, 0, "roip/%s", o->name)))
		return NULL;
	ast_channel_stage_snapshot(c);
	ast_channel_tech_set(c, &roip_tech);
	if (o->sounddev < 0)
	{
	...
	ast_channel_nativeformats_set(c, roip_tech.capabilities);
...
/*
 * COR edge handler: on a rising edge, originate a channel into the
 * dialplan at (extension,context) from roip.conf and start the PBX
 * on it. On a falling edge, optionally hang the COR-originated call up.
 * Called from the serial thread (serial DSR polling).
 */
static void 				roip_cor_event(struct chan_roip_pvt *o, int carrier)
{
	if (o == NULL || !o->cor_start_call)
		return;
	ast_mutex_lock(&alsalock);
	if (carrier && o->owner == NULL)
	{
		ast_verbose("== " ANSI_COLOR_GREEN "COR active on %s: starting PBX call at %s@%s" ANSI_COLOR_RESET "\n",
			o->name, o->ext, o->ctx);
		/* state != AST_STATE_DOWN makes roip_new() run ast_pbx_start() */
		if (roip_new(o, AST_STATE_RING, NULL, NULL) == NULL)
			ast_log(LOG_WARNING, "COR: unable to start PBX call on %s\n", o->name);
		else
			o->corcall = 1;
	}
	else if (!carrier && o->owner && o->corcall && o->cor_hangup)
	{
		ast_verbose("== " ANSI_COLOR_YELLOW "COR dropped on %s: hanging up %s" ANSI_COLOR_RESET "\n",
			o->name, ast_channel_name(o->owner));
		o->corcall = 0;
		ast_queue_hangup(o->owner);	/* thread-safe deferred hangup */
	}
	ast_mutex_unlock(&alsalock);
}
...
/*
*/
static struct ast_channel 		*roip_request(
	const char *type,
...
{
	struct ast_channel 			*c = NULL;
	struct chan_roip_pvt 	*o = find_desc(data);
	struct ast_str 				*codec_buf = ast_str_alloca(AST_FORMAT_CAP_NAMES_LEN);

	if (o->debuglevel)
		ast_log(LOG_NOTICE, "roip_request type:%s, dev:%s, format:%s\n",
	...
	if (o->owner)
	{
		ast_log(LOG_NOTICE, "Already have a call (channel %s <%p>) on the roip channel\n",
			(char *) o->name, o->owner);
		*cause = AST_CAUSE_BUSY;
		return NULL;
	}
	ast_mutex_lock(&alsalock);
	if ((c = roip_new(o, AST_STATE_DOWN, assignedids, requestor)) == NULL)
		ast_log(LOG_ERROR, "Unable to create new roip channel\n");
...
/*
 * Manually force PTT on/off from the CLI, independently of any call.
 * Uses the same txtestkey path as "roip key"/"roip unkey", so it
 * routes through manage_ptt() and therefore honors force_hardware_eptt
 * and invertptt, and kickptt() wakes the serial thread immediately.
 * Note: like key/unkey this keys the transmitter carrier only; it does
 * not by itself pass bridged call audio (that is what ptt_on_call is for).
 */
static int 						console_ptt(int fd, int argc, const char *const *argv)
{
	struct chan_roip_pvt 	*o;
	...
	if (!strcasecmp(argv[2], "on"))
		...
		return RESULT_SHOWUSAGE;
	kickptt(o);
	ast_cli(fd, "[%s] PTT forced %s\n", o->name, o->txtestkey ? "ON" : "OFF");
	return RESULT_SUCCESS;
}
...
{
	struct chan_roip_pvt 	*o = find_desc(roip_active);

	o->debuglevel=1;
	ast_cli(fd,"roip debug on.\n");
	return RESULT_SUCCESS;
}
...
static int 						radio_active(int fd, int argc, const char *const *argv)
{
    ...
    else
    {
        ...
        {
            for (o = roip_default.next; o; o = o->next)
...
static char ptt_usage[] =
	"Usage: roip ptt on|off\n"
	"       Manually key (on) or unkey (off) the transmitter PTT on the active\n"
	"       device, independently of any call. Equivalent to 'roip key'/'unkey'\n"
	"       but with an explicit on|off argument.\n";

static char command_usage[] =
	"Usage: roip command <command>\n"
	"       Send a control command to active radio.\n";


static char debug_usage[] =
	"Usage: roip debug [off]\n"
...
static void store_rxcdtype(struct chan_roip_pvt *o, char *s)
{
	if (!strcasecmp(s,"no")){
...
/*
*/
static void store_rxsdtype(struct chan_roip_pvt *o, char *s)
{
	if (!strcasecmp(s,"no")){
 ...
 */
static struct chan_roip_pvt	*store_config(struct ast_config *cfg, char *ctg)
{
	struct ast_variable 			*v;
	struct chan_roip_pvt 		*o;
	...
	for (v = ast_variable_browse(cfg, ctg); v; v = v->next) {
		...
		if (!ast_jb_read_conf(&global_jbconf, v->name, v->value))
			...
			M_BOOL("cor_start_call", o->cor_start_call)
			M_BOOL("cor_hangup", o->cor_hangup)
			M_BOOL("ptt_on_call", o->ptt_on_call)
			M_END(;
			);
	}
	o->debuglevel = 0;
	if (o == &roip_default)		/* we are done with the default */
		return NULL;
...
static char *handle_console_ptt(struct ast_cli_entry *e,
	int cmd, struct ast_cli_args *a)
{
        switch (cmd) {
        case CLI_INIT:
	...
	return res2cli(console_ptt(a->fd,a->argc,a->argv));
}
	...
	int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		e->command = "roip rkey";
		e->usage = "Usage: roip rkey\n       Simulates radio COR active on the active device.\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
	}
	sim_cor = 1;
	roip_cor_event(find_desc(roip_active), 1);
	return CLI_SUCCESS;
}
	...
	int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		e->command = "roip runkey";
		e->usage = "Usage: roip runkey\n       Simulates radio COR inactive on the active device.\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
	}
	sim_cor = 0;
	roip_cor_event(find_desc(roip_active), 0);
	return CLI_SUCCESS;
}

static struct ast_cli_entry cli_roip[] = {
	AST_CLI_DEFINE(handle_console_key,"Radio PTT key"),
	AST_CLI_DEFINE(handle_console_unkey,"Radio PTT unkey"),
	AST_CLI_DEFINE(handle_console_ptt,"Radio PTT on/off"),
	AST_CLI_DEFINE(handle_console_command,"Radio send command"),
	AST_CLI_DEFINE(handle_console_rkey,"Radio COR active (simulated)"),
	AST_CLI_DEFINE(handle_console_runkey,"Radio COR inactive (simulated)"),
	//AST_CLI_DEFINE(handle_aradio_tune,"roip Tune"),
	AST_CLI_DEFINE(handle_aradio_debug,"roip Debug On"),
	AST_CLI_DEFINE(handle_aradio_debug_off,"roip Debug Off"),
	AST_CLI_DEFINE(handle_aradio_active,"Change commanded device")
};

static snd_pcm_t 				*alsa_card_init(struct chan_roip_pvt *o,
												char *dev,
...
static int 				serial_init(struct chan_roip_pvt *o)
{
	...
	/* Raw serial: 8N1, no flow control, ignore parity errors */
...
// static int serial_getctcss(struct chan_roip_pvt *o)
// {
// 	int ret, status;
 ...
 */
static int 		serial_pttkey(struct chan_roip_pvt *o, enum ptt_status ptt)
{
	int 		ret;
	int 		status;
...
static int 						load_module(void)
{
	...
	roip_active = NULL;
	if (!(roip_tech.capabilities = ast_format_cap_alloc(AST_FORMAT_CAP_FLAG_DEFAULT)))
		return AST_MODULE_LOAD_DECLINE;
	ast_format_cap_append(roip_tech.capabilities, ast_format_slin, 0);
	...
	if (ast_channel_register(&roip_tech)) {
		ast_log(LOG_ERROR, "Unable to register channel type 'roip'\n");
	...
	ast_cli_register_multiple(cli_roip, sizeof(cli_roip) / sizeof(struct ast_cli_entry));
...
static int 						unload_module(void)
{
	...
	ast_channel_unregister(&roip_tech);
	ast_cli_unregister_multiple(cli_roip, sizeof(cli_roip) / sizeof(struct ast_cli_entry));
	for (o = roip_default.next; o; o = o->next) {
		#if DEBUG_CAPTURES == 1
		if (frxcapraw) { fclose(frxcapraw); frxcapraw = NULL; }
		if (ftxcapraw) { fclose(ftxcapraw); ftxcapraw = NULL; }
	...
	ao2_cleanup(roip_tech.capabilities);
	roip_tech.capabilities = NULL;
	ast_log(LOG_NOTICE, "Module unloaded\n");
	return 0;
}

AST_MODULE_INFO_STANDARD(ASTERISK_GPL_KEY, "Radio over IP (RoIP) Channel Driver");
